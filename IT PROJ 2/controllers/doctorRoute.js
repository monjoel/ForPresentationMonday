  ...
  app.get('/doctor/outpatientManagement', function(req, res){
    if(req.session.email && req.session.sino == 'doctor'){
      if (req.session.sino == 'doctor') {
          ...
          db.query(outpatientDepartmentSQL + availableBeds + labSQL + prescribeSQL + outpatientDepartmentSQL2 + outpatientDepartmentSQL3 + name + requestConfirmation + bedDisabble + requestStatusSQL + pendingLabRequestSQL + listMed,req.session.Aid, function(err, rows){
          if (err) {
            console.log(err);
          } else {
            if (rows[12] == "") {
              ...
              res.render('doctor/outpatientManagement', {opdInfo:rows[0], admitAvailableBeds:rows[1], labSQL:rows[2], prescribeSQL:rows[3], opdInfo1:rows[4], opdInfo2:rows[5], username: rows[6], request:rows[7], bedDisable:rows[8], requestStatus:rows[9],pendingLabRequestSQL:rows[10], listMed:rows[11], invalid:null});
          }
        });
      } else {
        res.redirect(req.session.sino+'/dashboard');
    ...
    if(req.session.email && req.session.sino == 'doctor'){
      if (req.session.sino == 'doctor') {
          if (data.sub == 'admit') {
            ...
            db.query('UPDATE patient_history set request = "pending", pharm_confirm = "pending", lab_confirm = "pending" where histo_id = '+req.query.id+';'
              ...
              if (err) {
                ...
                io.emit('type', {what:'requestConfirmation',message:'Request of confirmation for patient '+req.query.name+', sent by Dr. <strong>'+req.session.name+'</strong>'});
                res.redirect(req.get('referer'));
              }
            });
    ...
    if(req.session.email && req.session.sino == 'doctor'){
      if(req.session.sino == 'doctor') {
          if (data.sub == 'setUnavailable') {
              ...
              db.query(setBedUnavailable + 'INSERT into activity_logs(account_id, time, type, remarks) VALUES ('+req.session.Aid+',"'+moment(new Date()).format('YYYY-MM-DD HH:mm:ss')+'", "setBedUnavailable", "Set to unavailable bed number: '+req.query.bed+'");', function(err, rows){
                if (err) {
                  console.log(err);
                } else {
                  res.redirect(req.get('referer'));
      ...
      if(req.session.email && req.session.sino == 'doctor'){
        if(req.session.sino == 'doctor') {
          if (data.sub == 'changeInfo') {
            ...
            var med = "select lab_img,date_stamp, lab, medicine,diagnosis,bed, initial_assessment from patient_history where patient_id = "+req.query.patient_id+"  order by date_stamp desc;";
